...
object Day14 extends Year2018 {
  ...
  override def runDay(input: String): Unit = {
    val totalRecipes = input.toInt
    val searchedScore = input.map(_.asDigit)

    val (recipeScores, offset) = calculate(searchedScore, Vector(3, 7), 0, 1, Vector.fill(searchedScore.length + 1)(0))
  ...
  @tailrec
  def calculate(searchedScore: Seq[Int], recipeScores: Vector[Int], elf1: Int, elf2: Int, tailBuffer: Vector[Int]): (Vector[Int], Int) = {
    if (tailBuffer.take(searchedScore.length) == searchedScore) {
      ...
      calculate(searchedScore, newScores, (elf1 + elf1Score + 1) % newScores.length, (elf2 + elf2Score + 1) % newScores.length, newTail)
    }
  }
}
