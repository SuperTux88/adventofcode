...
object Day16 extends Year2019 {
  ...
  override def runDay(input: String): Unit = {
    val inputSignal = input.map(_.asDigit).toList

    val message100 = (1 to 100).foldLeft(inputSignal) { (state, _) =>
    ...
    val messageOffset = inputSignal.slice(0, 7).mkString.toInt
    val inputAfterOffset = List.fill(10000)(inputSignal).flatten.slice(messageOffset, inputSignal.length * 10000)

    val message100AfterOffset = (1 to 100).foldLeft(inputAfterOffset) { (state, _) =>
