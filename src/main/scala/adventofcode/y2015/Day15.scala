...
object Day15 extends Year2015 {
  ...
  private val possibilities = List.fill(ingredients.size)(0 to 100).flatten.combinations(ingredients.size)
  ...
  private val possibilitiesWith500Calories = possibilities.filter(p => p.map(i => i._2.calories*i._1).sum == 500)
